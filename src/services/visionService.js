...
function toBase64(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () =>
      reject(new Error("No se pudo leer el archivo. Intenta de nuevo."));
...
async function analyzeWithGemini(base64, mimeType) {
  ...
  try {
    ...
    if (err?.message?.includes("API key"))
      throw new Error(
        "API key de Gemini incorrecta. Verifica tu configuración.",
      );

    throw new Error(
      "Error al analizar el documento con Gemini. Intenta de nuevo.",
    );
  }

  const text = result.response.text();
  if (!text)
    throw new Error("Gemini no devolvió una respuesta. Intenta de nuevo.");
  ...
  try {
    ...
    if (!match)
      throw new Error(
        "No se pudieron extraer los datos de la factura. Intenta con una imagen más clara.",
      );
