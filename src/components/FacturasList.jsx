  ...
  return (
    <section className="w-full mt-14">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {buscando && (
              <svg
                className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-emerald-500 animate-spin"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
          ...
          {!loading && !error && !buscando && (
            <span className="text-xs font-medium text-zinc-400 bg-zinc-800 border border-zinc-700 px-2.5 py-1 rounded-full whitespace-nowrap">
              {listaVisible.length}{" "}
              {hayBusquedaActiva ? "resultados" : "registros"}
            </span>
          )}
        </div>
      </div>
      ...
      {mostrarSkeleton && (
        <div className="flex flex-col gap-2">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3.5 grid grid-cols-4 gap-4 animate-pulse"
            >
      ...
      {!mostrarSkeleton &&
        ...
        listaVisible.length > 0 && (
          <div className="flex flex-col gap-2">
            <div className="hidden sm:grid grid-cols-[1fr_1fr_1fr_1fr_auto] gap-3 px-4 py-2 text-xs font-medium text-zinc-500 uppercase tracking-widest">
              <span>Proveedor</span>
              <span>N° Factura</span>
            ...
            {listaVisible.map((factura, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-1 w-16 justify-end md:opacity-0 md:group-hover:opacity-100 opacity-100">
                  {factura.archivo_url && (
                    <a
